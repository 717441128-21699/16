...
import { Trophy, Star, Swords, Heart, Users, Zap, Coins, Shield, ArrowLeft, Sparkles } from 'lucide-react';
...
const generateRewards = (result: 'victory' | 'defeat', stats: BattleStats): Reward => {
  const multiplier = result === 'victory' ? 1 : 0.3;
  const baseExp = 500 + stats.totalDamage / 10 + stats.kills * 50;
  const baseGold = 2000 + stats.totalDamage / 20 + stats.kills * 100;
  const baseRep = 50 + stats.kills * 10;
...
export default function BattleResult() {
  ...
  const state = (location.state as BattleResultState) ?? {
  ...
  const rewards = generateRewards(state.result, state.stats);
  ...
  const playerDamagePercent = Math.round((state.stats.totalDamage / (state.stats.totalDamage + fakeTeamStats.filter(s => !s.isPlayer).reduce((sum, s) => sum + s.damage, 0))) * 100);
              ...
              <div className="space-y-4">
                ...
                <div className="pt-2 border-t border-white/5">
                  ...
                  <div className="space-y-2">
                    {[...fakeTeamStats.filter(s => !s.isPlayer), { name: '曙光 (你)', damage: state.stats.totalDamage, heal: state.stats.totalHeal, kills: state.stats.kills, isPlayer: true }]
